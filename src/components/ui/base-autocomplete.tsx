...
const inputVariants = cva(
	`
    bg-input/30 flex w-full border border-input shadow-xs shadow-black/5 transition-[color,box-shadow] text-foreground placeholder:text-muted-foreground/80 
  ...
  `,
	{
		variants: {
			variant: {
				lg: "h-10 px-4 text-sm rounded-4xl [&~[data-slot=autocomplete-clear]]:end-2.5",
				md: "h-9 px-3 text-sm rounded-4xl [&~[data-slot=autocomplete-clear]]:end-2",
				sm: "h-8 px-2.5 text-xs rounded-4xl [&~[data-slot=autocomplete-clear]]:end-1.75",
			},
		},
		defaultVariants: {
			variant: "md",
...
function AutocompleteItem({
	...
	return (
		<AutocompletePrimitive.Item
			...
			className={cn(
				"text-foreground relative flex cursor-default select-none items-center gap-2 rounded-xl px-4 py-2 text-sm outline-hidden transition-colors",
				...
				"data-[highlighted]:before:inset-x-1 data-[highlighted]:before:inset-y-0 data-[highlighted]:before:z-[-1] data-[highlighted]:before:rounded-xl data-[highlighted]:before:bg-accent",
				className,
			)}
			{...props}
		/>
...
function AutocompleteClear({
	...
	return (
		<AutocompletePrimitive.Clear
			...
			className={cn(
				"absolute cursor-pointer top-1/2 -translate-y-1/2 rounded-xl opacity-70 ring-offset-background transition-opacity hover:opacity-100",
